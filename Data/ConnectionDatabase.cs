...
namespace DinoTrack.Data
{
    ...
    namespace SchoolInventoryApp.Data
    {
        internal class ConnectionDatabase
        {
            ...
            internal readonly string connectionString = "Host=localhost;Port=5432;Username=postgres;Password=7;Database=School_Inventory";
            ...
            public string GetCategoryNameById(int subCategoryId)
            {
                try
                {
                    using (var conn = GetConnection())
            ...
            public DataTable GetInventoryHistory(int itemId)
            {
            ...
            public void LogLocationChange(int itemId, int? oldLocationId, int? newLocationId)
            {
                try
                {
                    using (var conn = GetConnection())
            ...
            // Метод для получения всех записей инвентаря из базы данных
            ...
            // Метод для получения записи инвентаря по ID
            ...
            public DateTime? GetInventoryStartDate(int inventoryId)
            {
                try
                {
                    using (var conn = GetConnection())
            ...
            public bool WriteOffInventory(int inventoryId)
            {
                try
                {
                    using (var conn = GetConnection())
            ...
            public bool RestoreInventory(int inventoryId)
            {
                try
                {
                    using (var conn = GetConnection())
